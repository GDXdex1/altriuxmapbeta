module altriux::altriuxanimal {
    ...
    const GATO_M: u8 = 62;
    const GATO_F: u8 = 63;
    const ELEFANTE: u8 = 64;       // from ELEFANTE_SALVAJE
    ...
    public fun feed_animal(animal: &mut Animal, inv: &mut Inventory, amount_jax: u64, clock: &Clock) {
        ...
        let required = get_daily_requirement_jax(animal.animal_type); 
        let amount_needed = required * 7;
    ...
    fun get_daily_requirement_jax(t: u8): u64 {
        if (t == TORO || t == VACA || t == BUFALO_MACHO || t == BUFALO_HEMBRA || t == CABALLO || t == YEGUA || t == CEBU_MACHO || t == CEBU_HEMBRA) 1
        else if (t == OVEJA_MACHO || t == OVEJA_HEMBRA || t == CABRA_MACHO || t == CABRA_HEMBRA || t == BURRO || t == BURRA) 1 
        else 1
    }
