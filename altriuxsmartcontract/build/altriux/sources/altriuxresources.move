module altriux::altriuxresources {
    ...
    const TABLON_ANCHO: u64 = 140;
    const TABLON_SEGUNDA: u64 = 141;
    const VIGA_LARGA: u64 = 142;
    const CARBON_MADERA: u64 = 143;
    const LENA_SECA: u64 = 144;
    const ASTILLAS_MADERA: u64 = 145;
    const VIRUTAS_MADERA: u64 = 146;
    const TABLAS_MADERA: u64 = 147;
    const VIGAS_MADERA: u64 = 148;
    const MADERA_LAMINADA: u64 = 149;
    const MADERA_TRATADA: u64 = 150;
    const RESINA_MADERA: u64 = 151;
    const MADERA_BAMBU: u64 = 152;
    ...
    // --- Industrial/Beekeeping (213-214) ---
    const CERA_ABEJA: u64 = 213;
    const RESERVA_214: u64 = 214;
    
    // --- Ship Components (215-220, 225) ---
    ...
    const JAX_VELA_ITEM: u64 = 220; 
    const ESTOPAS: u64 = 225;

    // --- Writing Materials (221-224) ---
    const PAPEL: u64 = 221;
    const TINTA: u64 = 222;
    const PLUMA_ESCRIBIR: u64 = 223;
    const CONCHA_VEGETAL: u64 = 224;
    ...
    const HILO_ALGODON: u64 = 166;
    const HILO_LINO: u64 = 167;
    const HILO_CANAMO: u64 = 168;
    const HILO_LANA: u64 = 169;
    const HILO_SEDA: u64 = 170;
    const HILO_YAK_FINO: u64 = 171;
    const HILO_YAK_GRUESO: u64 = 172;
    const HILO_CACHMIRA: u64 = 173;
    const HILO_ALPACA: u64 = 174;
    
    const TELA_LINO_NATURAL: u64 = 181;
    const TELA_LANA_NATURAL: u64 = 182;
    const TELA_ALGODON_NATURAL: u64 = 183;
    const TELA_CANAMO_NATURAL: u64 = 184;
    
    const TINTE_OCRE_ROJO: u64 = 201;
    const MORDIENTE_HIERRO: u64 = 205;

    // --- Special items (226-227) ---
    ...
    public fun add_jax(inv: &mut Inventory, type_id: u64, amount: u64, expiry_days: u64, clock: &Clock) {
        ...
        let (w_unit, v_unit) = if (type_id >= 301 && type_id <= 362) {
            altriuxitems::get_item_metrics(type_id)
        } else if (type_id == MONEDA_ORO) {
            (20, 1)
        } else {
    ...
    public fun consume_jax(inv: &mut Inventory, type_id: u64, amount: u64, clock: &Clock) {
        ...
        assert!(kingdomutils::get_game_time(clock) < expiry || expiry == 0, E_EXPIRED);
    ...
    public fun transfer_jax(from: &mut Inventory, to: &mut Inventory, type_id: u64, amount: u64, clock: &Clock) {
    ...
    // --- Production Logic ---
    public fun produce_paper(inv: &mut Inventory, amount: u64, clock: &Clock) {
        consume_jax(inv, ESTOPAS, amount * 2, clock);
        add_jax(inv, PAPEL, amount, 0, clock); 
    }

    public fun produce_ink(inv: &mut Inventory, amount: u64, clock: &Clock) {
        consume_jax(inv, TANINO, amount, clock);
        consume_jax(inv, CONCHA_VEGETAL, amount, clock);
        add_jax(inv, TINTA, amount, 0, clock);
    }

    public fun produce_tow(inv: &mut Inventory, amount: u64, clock: &Clock) {
        let flax_id = LINO_SIN_HILAR; 
        consume_jax(inv, flax_id, amount, clock);
        add_jax(inv, ESTOPAS, amount * 3, 0, clock);
    }

    public fun produce_tannin(inv: &mut Inventory, amount: u64, clock: &Clock) {
        consume_jax(inv, ASTILLAS_MADERA, amount * 5, clock);
        add_jax(inv, TANINO, amount, 0, clock);
    }

    // Getters for book module
    public fun id_papel(): u64 { PAPEL }
    public fun id_tinta(): u64 { TINTA }
    public fun id_pluma(): u64 { PLUMA_ESCRIBIR }
