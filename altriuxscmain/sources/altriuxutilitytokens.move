...
module altriux::esc {
    ...
    public struct ESCTreasury has key {
        id: UID,
        cap: TreasuryCap<ESC>,
        minted: u128,
    ...
    fun init(witness: ESC, ctx: &mut TxContext) {
        let (treasury_cap, metadata) = coin::create_currency<ESC>(
            witness,
            DECIMALS,
            b"ESC",
            b"Escudo Silver Coin",
            b"Silver coin (20g Ag)",
            option::none(),
            ctx
        );
        transfer::public_freeze_object(metadata);
        let treasury = ESCTreasury {
            id: object::new(ctx),
            cap: treasury_cap,
            minted: 0,
            remint_pool: 0,
    ...
    public fun mint(
        ...
        if (!is_admin) {
            // ESC: 20g Silver (Simplified for ESC)
        ...
        // Weight check: ESC is 20g, Volume is ~2mL
    ...
    public fun burn_for_contrabandista(treasury: &mut ESCTreasury, coins: Coin<ESC>) {
    ...
    public fun remint_from_pool(_: &AdminCap, treasury: &mut ESCTreasury, amount: u64, recipient: address, ctx: &mut TxContext) {
        let amount_u128 = (amount as u128);
