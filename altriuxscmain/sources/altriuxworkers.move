...
module altriux::altriuxworkers {
    ...
    // --- Constants ---
    const COST_ANNUAL_ESC: u64 = 40_000_000_000; // 40 ESC (9 decimals)
    const COST_TEMP_ESC: u64 = 4_000_000_000;    // 4 ESC
    ...
    // --- Worker Roles ---
    public fun ROLE_GENERAL(): u8 { 0 }
    public fun ROLE_MINERO(): u8 { 1 }
    public fun ROLE_HERRERO(): u8 { 2 }
    public fun ROLE_CARPINTERO(): u8 { 3 }
    public fun ROLE_CONSTRUCTOR(): u8 { 4 }
    public fun ROLE_TEJEDOR(): u8 { 5 }
    public fun ROLE_SASTRE(): u8 { 6 }
    public fun ROLE_PANADERO(): u8 { 7 }
    public fun ROLE_CARNICERO(): u8 { 8 }
    public fun ROLE_ARTESANO(): u8 { 9 }
    ...
    public fun hire_worker(
        ...
        let cost = if (is_annual) COST_ANNUAL_ESC else COST_TEMP_ESC;
    ...
    public fun action_construct(_worker: &mut WorkerContract, _building_id: u64) {
        // Implementation pending
    }

    public fun action_harvest(_worker: &mut WorkerContract, _tile_id: u64) {
        // Implementation pending
    }
    ...
    public fun borrow_worker(reg: &WorkerRegistry, worker_id: ID): &WorkerContract {
        table::borrow(&reg.active_contracts, worker_id)
    }
    ...
    public fun get_worker_role(contract: &WorkerContract): u8 {
        contract.role
    }

    public fun set_worker_active(contract: &mut WorkerContract, active: bool) {
        contract.is_active = active;
    }

    public fun get_active(contract: &WorkerContract): bool {
        contract.is_active
    }
}
