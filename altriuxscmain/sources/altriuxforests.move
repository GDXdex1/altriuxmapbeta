...
module altriux::altriuxforests {
    ...
    use altriux::altriuxlocation::{Self, LocationRegistry, ResourceLocation, encode_coordinates, decode_coordinates, hex_distance, is_adjacent, has_feature, feature_forest, feature_jungle, feature_boreal_forest};
    ...
    // === CREACIÓN DE BOSQUE (Basado en característica existente) ===
    public fun create_forest_from_location(
        ...
        // Validar que existe bosque en esta ubicación (como característica)
        let forest_type: u8 = if (has_feature(loc_reg, q, r, feature_forest())) {
            FOREST_TYPE_TEMPERATE
        } else if (has_feature(loc_reg, q, r, feature_jungle())) {
            FOREST_TYPE_TROPICAL
        } else if (has_feature(loc_reg, q, r, feature_boreal_forest())) {
            FOREST_TYPE_BOREAL
        } else {
